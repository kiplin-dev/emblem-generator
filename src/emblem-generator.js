...
emblemGenerator.init = function(id, size, bgColor) {
	emblemGenerator.paper = Raphael(id, size, size);

	// used for shadow over color
	emblemGenerator.pto2_color = '#000000';
	emblemGenerator.pto2_op = 0.3;

	// used for cross color transparency
	emblemGenerator.pt1_op = 0.3;

	// used for emblem background
	emblemGenerator.bg_op = 1;

	// paper background
	emblemGenerator.bg_color = bgColor || '';
	emblemGenerator.bg_img = 'assets/img_bg.png';

	// config required for transformation
	emblemGenerator.base_size = 256;
	emblemGenerator.flip = 0; // 1 - flipV_Bg, 2 - flipH_Bg, 4 - flipV_Fg, 8 - flipH_Fg

	return emblemGenerator;
};
...
*/
emblemGenerator.drawEmblemObj = function(EGobj) {
	emblemGenerator.setFlipsEG(EGobj.flags);

	var colorBg = emblemGenerator.color_defs[EGobj.background_color_id] || '#000000',
		color1 = emblemGenerator.color_defs[EGobj.foreground_secondary_color_id] || '#FFFFFF',
		color2 = emblemGenerator.color_defs[EGobj.foreground_primary_color_id] || '#FF0000';

	var defFg = emblemGenerator.defs[EGobj.foreground_id] || '',
		defBg = emblemGenerator.bg_defs[EGobj.background_id] || '';

	emblemGenerator.drawEmblem(defFg, color1, color2, defBg, colorBg);
};

emblemGenerator.drawEmblem = function(defFg, color1, color2, defBg, colorBg) {
	var paper = emblemGenerator.paper;

	paper.clear();

	// set background
	if (emblemGenerator.bg_color !== '')
		paper.rect(0, 0, paper.width, paper.height).attr({'fill':emblemGenerator.bg_color, 'stroke':emblemGenerator.bg_color});
	else
		paper.image(emblemGenerator.bg_img, 0, 0, paper.width, paper.height);

	emblemGenerator.drawEmblemBg(defBg, colorBg);
	emblemGenerator.drawEmblemFg(defFg, color1, color2);
};

emblemGenerator.drawEmblemFg = function(def, color1, color2) {
	var paper = emblemGenerator.paper,
		i;

	var scale = paper.width / emblemGenerator.base_size,
		transformStr = (scale != 1) ? 's'.concat(scale, ',', scale, ',0,0') : '';

	if (emblemGenerator.flip > 3)
	{
		transformStr = transformStr.concat(' s',((emblemGenerator.flip & 8) !== 0) ? -1 : 1,',',((emblemGenerator.flip & 4) !== 0) ? -1 : 1,',',emblemGenerator.base_size/2,',',emblemGenerator.base_size/2);
	}

	emblemGenerator.paths = [];
	var paths = emblemGenerator.paths;
	if (def.p1)
	{
	...
	if (def.pto2)
	{
		for(i=0;i<def.pto2.length;i++)
			paths[paths.length] = paper.path(def.pto2[i]).attr({'fill':emblemGenerator.pto2_color, 'stroke':'none', 'opacity':emblemGenerator.pto2_op}).transform(transformStr);
	}

	if (def.pt1)
	{
		for(i=0;i<def.pt1.length;i++)
			paths[paths.length] = paper.path(def.pt1[i]).attr({'fill':color1, 'stroke':'none', 'opacity':emblemGenerator.pt1_op}).transform(transformStr);
	}

	return paths;
};

emblemGenerator.drawEmblemBg = function(def, color) {
	var paper = emblemGenerator.paper,
		i,
		opacity = def.t ? emblemGenerator.bg_op : 1;

	var scale = paper.width / emblemGenerator.base_size,
		transformStr = (scale != 1) ? 's'.concat(scale, ',', scale, ',0,0') : '';

	if ((emblemGenerator.flip & 1) !== 0 || (emblemGenerator.flip & 2) !== 0)
	{
		transformStr = transformStr.concat(' s',((emblemGenerator.flip & 2) !== 0) ? -1 : 1,',',((emblemGenerator.flip & 1) !== 0) ? -1 : 1,',',emblemGenerator.base_size/2,',',emblemGenerator.base_size/2);
	}

	emblemGenerator.bg_paths = [];

	var paths = emblemGenerator.bg_paths;
	if (def.p)
	{
